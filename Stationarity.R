...
plot(diff(Tbrate[,1]))
pacf(Tbrate[,1])
adf.test(diff(Tbrate[,1]))
adf.test(diff(Tbrate[,2]))
adf.test(diff(Tbrate[,3]))
...
fit1 = arima(Tbrate[,3],order=c(0,1,1))
acf(residuals(fit1))
...
#the T-bill series.
fit2 = arima(diff(Tbrate[,3]),order=c(0,1,1))
acf(residuals(fit2))
Box.test(residuals(fit2), lag = 10, type="Ljung")
